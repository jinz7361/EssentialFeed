...
//  EssentialFeedTests
//
//  Created by Jin Zhang on 2/3/24.
//
...
class CoreDataFeedImageDataStoreTests: XCTestCase {
    func test_retrieveImageData_deliverNotFoundWhenEmpty() {
        let sut = makeSUT()
        
        expect(sut, toCompleteRetrievalWith: notFound(), for: anyURL())
    }
    
    func test_retrievalImageData_deliversNotFoundWhenStoredDataURLDoesNotMatch() {
        let sut = makeSUT()
    ...
    private func insert(_ data: Data, for url: URL, into sut: CoreDataFeedStore, file: StaticString = #file, line: UInt = #line) {
        ...
        sut.insert([image], timestamp: Date()) { result in
            ...
            case let .failure(error):
                ...
                exp.fulfill()
            case .success:
                sut.insert(data, for: url) { resule in
                    if case let Result.failure(error) = resule {
